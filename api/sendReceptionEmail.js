export default async function handler(req, res) {
  ...
  try {
    ...
    if (!to.includes('@')) return res.status(400).json({ ok:false, error:'Correo inválido' });
    const response = await fetch('https://api.resend.com/emails', {
      method:'POST',
      headers:{ Authorization:`Bearer ${apiKey}`, 'Content-Type':'application/json' },
      body: JSON.stringify({ from:`The Clinic <${fromEmail}>`, to, subject:'The Clinic - Recepción', html:'<h2>The Clinic</h2><p>Recepción registrada.</p>' })
    });
